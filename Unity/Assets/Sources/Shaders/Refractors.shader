Shader "Game/Refractors" {

	Properties{
		_Color("Tint", Color) = (1,1,1,1)
		_Background("Background", 2D) = "white" {}			//Receive input from a Texture on Texture Unit 0
...
#include "UnityCG.cginc"		//Include Unity's predefined inputs and macros
	...
	fixed4 background = _Color * tex2D(_Background, float2(_BackgroundScaleX,_BackgroundScaleY)*i.uv + (_Refraction*(distortion + distortion2)) + float2(_BackgroundScrollX,_BackgroundScrollY));	//Get the background
	fixed4 background2 = _Color2 * tex2D(_Background2, float2(_BackgroundScaleX2,_BackgroundScaleY2)*i.uv + (_Refraction2*(distortion + distortion2)) + float2(_BackgroundScrollX2,_BackgroundScrollY2));	//Get the background
	fixed alpha = background.a*_BackgroundFade2;
	fixed inverse = 1.0 - alpha;
	return fixed4((background.r*alpha) + (background2.r*inverse),(background.g*alpha) + (background2.g*inverse),(background.b*alpha) + (background2.b*inverse),1.0);	//Return alphablended backgrounds
	}
